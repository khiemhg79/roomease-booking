...
export default function MyBookingsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (completedIds.length) {
        const status =
          await reviewApi.status(
            completedIds,
          )

        setReviewedBookingIds(
          new Set(
            status?.reviewedBookingIds || [],
          ),
        )
      } else {
        setReviewedBookingIds(
  ...
  const cancel = async (
    ...
    if (
      !window.confirm(
        `Bạn chắc chắn muốn hủy booking ${booking.bookingCode}?`,
      )
    ) {
      return
    }
    ...
    try {
      ...
      setNotice(
        `Đã hủy booking ${booking.bookingCode}.`,
      )
    ...
    <main className="container page-section customer-bookings-page">
      <section className="booking-page-hero">
        <div>
          ...
          <p>
            Theo dõi trạng thái, chuẩn bị chuyến đi
            và đánh giá sau khi lưu trú.
          </p>
        </div>
      ...
      {!bookings.length ? (
        <section className="empty-state booking-empty-state">
          ...
          <p>
            Tìm chỗ nghỉ phù hợp và bắt đầu chuyến đi đầu tiên.
          </p>
        ...
        <section className="empty-state">
          <h2>
            Không có booking trong nhóm này
          </h2>
                ...
                >
                  ...
                  <div className="booking-card-body">
                    <div className="booking-property-copy">
                      <Link
                        to={`/property/${booking.propertySlug}`}
                      >
                        <h2>
                      ...
                      <div className="booking-date-flow">
                        ...
                        <span className="booking-night-pill">
                          {booking.nights} đêm
                        </span>
                  ...
                  {booking.cancellationDeadline
                    && ['PENDING', 'CONFIRMED']
                      ...
                      <div
                        className={
                          `booking-cancel-note ${cancelAllowed
                            ? ''
                            : 'expired'
                          }`
                        }
