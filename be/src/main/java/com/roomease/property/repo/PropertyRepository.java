...
public interface PropertyRepository
    ...
    Optional<Property> findBySlugAndStatus(
        String slug,
    ...
    List<Property>
    findTop8ByStatusAndFeaturedTrueOrderByReviewScoreDesc(
        PropertyStatus status
    );

    List<Property>
    findByOwner_IdOrderByNameAsc(
        UUID ownerId
    );
    ...
    @Query("""
        ...
        where
            ...
            and (
                ...
                or lower(coalesce(owner.email, ''))
                    like concat('%', :keyword, '%')

                or lower(coalesce(owner.fullName, ''))
                    like concat('%', :keyword, '%')
            )
