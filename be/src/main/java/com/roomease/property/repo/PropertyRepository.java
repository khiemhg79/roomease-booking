...
public interface PropertyRepository extends JpaRepository<Property, UUID> {
    ...
    List<Property> findTop8ByStatusAndFeaturedTrueOrderByReviewScoreDesc(
    ...
    @Query("""
        ...
        where (:status is null or p.status = :status)
          and (
            ...
            or lower(p.city) like concat('%', :keyword, '%')
            or lower(p.slug) like concat('%', :keyword, '%')
            or lower(coalesce(owner.email, '')) like concat('%', :keyword, '%')
            or lower(coalesce(owner.fullName, '')) like concat('%', :keyword, '%')
          )
